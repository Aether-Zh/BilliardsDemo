...
def copy_prebuilts(app_android_root):

    armeabi_dir = os.path.join(app_android_root, "libs/armeabi")
    prebuilts_dir = os.path.join(app_android_root, "jni/prebuilts")
    if os.path.isdir(armeabi_dir) and os.path.isdir(prebuilts_dir):
        copy_files(prebuilts_dir, armeabi_dir)
...
def build(ndk_build_param,android_platform,build_mode):
    ...
    copy_prebuilts(app_android_root)
